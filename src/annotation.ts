...
export class Annotation extends Script {
    ...
    static hoverColor = new Color(0.859, 0.078, 0.42);   // Lumina brand pink (#DB146B)
    ...
    initialize() {
        ...
        // Hide the in-scene numbered hotspot visuals (the 3D dots) to reduce clutter.
        // The annotation entity and its data remain intact, so the top navigation bar
        // can still fly the camera to each spot via the annotation array.
        base.enabled = false;
        ...
        // Hit-target disabled: the 3D markers are hidden, so the in-scene hotspots are
        // non-interactive. Navigation is driven solely by the top navigation bar.
        this.hotspotDom.style.pointerEvents = 'none';
