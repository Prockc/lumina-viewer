...
const initUI = (global: Global) => {
    ...
    events.on('progress:changed', (progress) => {
        ...
        if (progress < 100) {
            dom.loadingBar.style.backgroundImage = `linear-gradient(90deg, #DB146B 0%, #DB146B ${progress}%, #E3E3E3 ${progress}%, #E3E3E3 100%)`;
        } else {
            dom.loadingBar.style.backgroundImage = 'linear-gradient(90deg, #DB146B 0%, #DB146B 100%)';
        }
    });
    ...
    // Auto-show help panel once when the model finishes loading.
    // Tab is pre-selected based on device type so the user sees relevant controls immediately.
    events.once('loaded:changed', () => {
        const isTouchDevice = navigator.maxTouchPoints > 0;
        updateInfoTab(isTouchDevice ? 'touch' : 'desktop');
    ...
    updateCameraModeUI(); // apply initial active state immediately
    ...
    dom.resetCamera.addEventListener('click', (event) => {
        events.fire('inputEvent', 'resetFly', event);
        state.gamingControls = true;
    });
    ...
    tooltip.register(dom.flyCamera, 'First-Person Camera', 'top');
    ...
    if (window.parent !== window && isThirdPartyEmbedded() && dom.supersplatBranding) {
