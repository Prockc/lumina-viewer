...
/**
 * Smooth ease-in-out (Perlin smootherstep) for cinematic transitions that
 * accelerate and decelerate gently at both ends.
 *
 * @param x - Input value in the range [0, 1].
 * @returns - Output value in the range [0, 1].
 */
const easeInOut = (x: number) => x * x * x * (x * (x * 6 - 15) + 10);
...
export { damp, easeOut, easeInOut, mod, nearlyEquals, vecToAngles };
